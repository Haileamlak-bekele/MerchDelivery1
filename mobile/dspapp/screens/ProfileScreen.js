  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // 1. Fetch DSP profile
        const res = await fetch('http://192.168.4.182:5000/dsp/profile', {
          headers: {
        ...
        if (res.ok && dspData.userId) {
          ...
          const userRes = await fetch(`http://192.168.4.182:5000/users/getUserById/${dspData?.userId}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json',
            },
...
<TouchableOpacity
  ...
  onPress={async () => {
    try {
      ...
      const res = await fetch(`http://192.168.4.182:5000/users/logout`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
