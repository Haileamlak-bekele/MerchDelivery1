...
export default function OrdersPage() {
  ...
  const getStatusPill = (status) => {
    switch (status) {
      ...
      case 'approved':
        return <span className="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 items-center"><CheckCircle size={14} className="mr-1.5" />Admin Approved - DSP Assigned</span>;
      case 'shipped':
      ...
      case 'DspAssigned':
        return <span className="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 items-center"><CheckCircle size={14} className="mr-1.5" />DSP Assigned</span>;
      default:
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8">
        ...
        <div className="md:hidden flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex items-center">
            <ShoppingCart className="w-7 h-7 mr-2 text-green-600" />
            Order Management
          </h1>
        </div>
              ...
              <table className="w-full min-w-[700px] text-sm text-left text-gray-700">
                ...
                <tbody>
                  {orders.length === 0 ? (
                    ...
                    orders.map((order) => (
                      <tr key={order._id} className="border-b border-gray-200 hover:bg-gray-50 transition duration-150 ease-in-out group">
...
function OrderDetailsModal({ isOpen, onClose, order, onConfirm, inventoryItems, activeDsps, updateOrderStatus }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4 transition-opacity duration-300 ease-in-out animate-fade-in">
      <div className="bg-white border border-gray-200 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-scale-in flex flex-col">
        <div className="flex justify-between items-center p-5 border-b border-gray-200 sticky top-0 bg-white z-10">
          <h3 className="text-xl font-semibold text-gray-900">Order Details: {order._id}</h3>
          <button
            onClick={onClose}
            className="p-1.5 rounded-full text-gray-600 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500"
        ...
        <div className="p-6 space-y-6 flex-grow">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="text-lg font-semibold text-green-600 mb-2">Customer Information</h4>
              <p><strong className="text-gray-600">Name:</strong> {order.customer?.name || 'N/A'}</p>
              <p><strong className="text-gray-600">Order Date:</strong> {new Date(order.createdAt).toLocaleString()}</p>
              <p>
            ...
            <div>
              <h4 className="text-lg font-semibold text-green-600 mb-2">Order Summary</h4>
              <p><strong className="text-gray-600">Status:</strong> {order.orderStatus}</p>
              <p><strong className="text-gray-600">Total Amount:</strong> ${order.totalAmount.toFixed(2)}</p>
              {order.dspAssigned && (
                <p>
                  <strong className="text-gray-600">DSP Assigned:</strong>{" "}
          ...
          <div>
            <h4 className="text-lg font-semibold text-green-600 mb-3">Items Ordered</h4>
            <ul className="space-y-3">
              {order.items.map((item, index) => {
                ...
                return (
                  <li key={index} className="p-3 bg-gray-50 rounded-md border border-gray-200 flex items-center gap-4">
                    <div className="flex-shrink-0 w-16 h-16">
                    <img
                                src={getImageUrl(item.product?.image)}
                                alt={item.product?.name || 'Product'}
                                className="w-16 h-16 object-cover rounded-md border border-gray-200 bg-gray-100"
                                onError={e => { e.target.onerror = null; e.target.src = 'https://placehold.co/64x64'; }}
                              />
                    </div>
                    <div className="flex-grow">
                      <p className="font-medium text-gray-900">{item.product?.name || 'Product'} (x{item.quantity})</p>
                      <p className="text-sm text-gray-600">Price per unit: ${item.product?.price?.toFixed(2) || '0.00'}</p>
                    </div>
                    <div className="text-right">
                      {inventoryItem ? (
                        <>
                          {isOutOfStock ? (
                            <span className="text-xs font-semibold px-2 py-1 rounded-full bg-red-100 text-red-600">Out of Stock</span>
                          ) : !hasEnoughStock ? (
                            <span className="text-xs font-semibold px-2 py-1 rounded-full bg-yellow-100 text-yellow-600">Low Stock (Avail: {availableStock})</span>
                          ) : (
                            <span className="text-xs font-semibold px-2 py-1 rounded-full bg-green-100 text-green-600">In Stock (Avail: {availableStock})</span>
                          )}
                        </>
                      ) : (
                        <></>
            ...
            {order.items.some(item => {
              ...
              <div className="mt-4 p-3 bg-yellow-100 border border-yellow-200 rounded-md text-yellow-700 text-sm flex items-center">
    ...
    >
      Assign DSP
        ...
        <div className="flex justify-end items-center pt-5 border-t border-gray-200 mt-auto space-x-3 sticky bottom-0 bg-white p-6 -mx-0 -mb-0 rounded-b-lg">
          <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-gray-100 transition duration-150 ease-in-out">
            Close
          </button>
          {order.orderStatus === 'PENDING' && (
            <button
              onClick={() => onConfirm(order._id)}
              className="px-5 py-2.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-100 transition duration-150 ease-in-out shadow-md hover:shadow-lg flex items-center"
            >
