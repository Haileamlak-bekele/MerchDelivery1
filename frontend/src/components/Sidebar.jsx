...
function Sidebar({ isSidebarOpen, setIsSidebarOpen }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 md:z-10 w-64 bg-white border-r border-gray-200 p-6 transform ${
      ...
      >
        <div className="flex items-center justify-between mb-10">
          ...
          <button className="md:hidden text-gray-600 hover:text-gray-900" onClick={() => setIsSidebarOpen(false)}>
            <ArrowLeft size={20} />
          </button>
        </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <li key={item.id} className="mb-3">
                  ...
                  >
                    <item.icon className={`w-5 h-5 mr-3 ${isActive ? 'text-white' : 'text-green-600'}`} />
        ...
        <div className="mt-auto">
          <button
            ...
            className="w-full flex items-center px-4 py-3 rounded-lg text-gray-700 hover:bg-gray-200 hover:text-gray-900 transition-colors duration-150 ease-in-out"
          >
            <LogOut className="w-5 h-5 mr-3 text-green-600" />
